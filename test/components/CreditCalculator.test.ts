...
describe('CreditCalculator', () => {
  describe('initial state', () => {
    ...
    it('defaults to monthly rate (TEM) mode', () => {
      const wrapper = mount(CreditCalculator)
      expect((wrapper.find('[data-testid="rate-type-select"]').element as HTMLSelectElement).value).toBe('monthly')
    })
  ...
  describe('rate type toggle', () => {
    it('switches to annual (TEA) when selected', async () => {
      ...
      expect((rateTypeSelect(wrapper).element as HTMLSelectElement).value).toBe('annual')
    })
