        ...
        <table class="table table-xs table-zebra w-full font-mono">
          <thead>
            <tr>
              ...
              <th v-if="showAdminFees" class="text-right">Gastos admin.</th>
          ...
          <tbody>
            <tr v-for="row in schedule" :key="row.month">
              ...
              <td v-if="showAdminFees" class="text-right text-error">{{ formatCOPDecimals(row.adminFees) }}</td>
          ...
          <tfoot>
            <tr class="font-bold border-t-2 border-base-300">
              ...
              <td></td>
              <td class="text-right text-warning">{{ formatCOPDecimals(schedule.reduce((s, r) => s + r.interestPayment, 0)) }}</td>
              <td class="text-right text-success">{{ formatCOPDecimals(schedule.reduce((s, r) => s + r.capitalPayment, 0)) }}</td>
              <td v-if="showAdminFees" class="text-right text-error">{{ formatCOPDecimals(schedule.reduce((s, r) => s + r.adminFees, 0)) }}</td>
